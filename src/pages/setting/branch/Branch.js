...
const Branch = ({ t, location }) => {
  ...
  useEffect(() => {
    ...
    dispatch(
      changeListButtonHeader([
        <Link to={"/setting/branch/newBranch"} className="btn btn-primary" key="newBranch">
          Tạo chi nhánh
        </Link>
      ])
    );
  ...
  return <CContainer fluid className="c-main mb-3 px-4">
    ...
    ) : <QTable columnDef={columnDef} data={data} route={"/setting/branch"} idxColumnsFilter={[0, 1]} />}
  </CContainer>;
};
